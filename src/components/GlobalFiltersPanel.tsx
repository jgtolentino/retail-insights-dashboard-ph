    ...
    <div className="flex flex-wrap items-start gap-6 rounded bg-white p-4 shadow">
      <div className="w-56">
        ...
        <Select
          ...
          onChange={(vals: MultiValue<any>) => setFilters({ categories: vals.map(v => v.value) })}
      ...
      <div className="w-56">
        ...
        <Select
          ...
          onChange={(vals: MultiValue<any>) => setFilters({ brands: vals.map(v => v.value) })}
      ...
      <div className="w-56">
        ...
        <Select
          ...
          onChange={(vals: MultiValue<any>) => setFilters({ products: vals.map(v => v.value) })}
      ...
      <div className="w-56">
        ...
        <Select
          ...
          onChange={(vals: MultiValue<any>) => setFilters({ locations: vals.map(v => v.value) })}
          placeholder="All locations…"
        />
      </div>

      <div className="w-56">
        <label className="mb-1 block text-sm font-medium text-gray-700">Income Ranges</label>
        <Select
          isMulti
          options={toOptions(allIncomes)}
          value={toOptions(filters.incomeRanges)}
          onChange={(vals: MultiValue<any>) => setFilters({ incomeRanges: vals.map(v => v.value) })}
