...
export function BehaviorSuggestionsTable({ startDate, endDate, storeId: initialStoreId }: BehaviorSuggestionsTableProps) {
  ...
  const fetchBehaviorData = async () => {
    ...
    setError(null)
    try {
      const behaviorData = await behavioralDashboardService.getBehaviorSuggestions(startDate, endDate, storeId)
  ...
  const exportToCSV = () => {
    const headers = ['Date', 'Store ID', 'Store Name', 'Region', 'Total Transactions', 'Suggestions Offered', 'Suggestions Accepted', 'Acceptance Rate (%)']
            ...
            <p className="text-2xl font-bold text-purple-900">
              {data.length > 0 
                ? (data.reduce((sum, d) => sum + d.suggestionsAccepted, 0) / 
                   data.reduce((sum, d) => sum + d.suggestionsOffered, 0) * 100).toFixed(1)
                : 0}%
            </p>
          </div>
        </div>
              ...
              {filteredData.length > 0 ? (
                filteredData.map((item, index) => (
                  <TableRow key={`${item.date}-${item.storeId}-${index}`}>
                    ...
                    <TableCell className="text-right">{item.totalTransactions.toLocaleString()}</TableCell>
                    <TableCell className="text-right">{item.suggestionsOffered.toLocaleString()}</TableCell>
                    <TableCell className="text-right">{item.suggestionsAccepted.toLocaleString()}</TableCell>
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        <span className="font-medium">{item.suggestionAcceptanceRate.toFixed(1)}%</span>
                        <Progress 
                          value={item.suggestionAcceptanceRate} 
                          className="w-16 h-2"
                        />
                      </div>
                    </TableCell>
