...
// Enhanced error checking with detailed debugging information
const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY;

// Detailed error checking for better debugging
if (!supabaseUrl || !supabaseAnonKey) {
  ...
  console.error('❌ Environment Variables Check:', {
    ...
    'Platform detected': typeof window !== 'undefined' ? 'Browser' : 'Server',
  });

  throw new Error(`Missing required Supabase environment variables: ${missingVars.join(', ')}. 

🔧 QUICK FIX:
1. Go to your deployment platform (Vercel, Netlify, Lovable, etc.)
